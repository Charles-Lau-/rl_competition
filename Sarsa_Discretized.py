...
class helicopter_agent(Agent):
	...
	def getQValueOfNextState(self,obs_key,next_key):
		if(self.Q_Table.has_key(next_key)):
			...
			if(self.randGenerator.random() > self.Epsilon):
				...
				Q_next = cand_actions[action_with_max_value]
			else:
				action_random = convertListToString(self.discretize_action(util.randomGaussianAction(self.lastAction.doubleArray)))
				Q_next = self.Q_Table[next_key].has_key(action_random) and cand_actions[action_random] or 0 		
		else:
			Q_next = 0
	...
	def agent_action_step(self,reward,observation):
		...
		Q_next = self.getQValueOfNextState(obs_key,next_key)
 
		#update step_size
		self.updateStepSize(obs_key,act_key)

		Q_s_a = Q_s_a +	1.0/self.Step_Size[obs_key][act_key]*(reward + Q_next - Q_s_a)
