...
class helicopter_agent(Agent):
	...
	def agent_action_step(self,reward,observation):
		...
		Q_s_a = actions.get(act_key,0)		
		
		
		#get   Q-value of next state and next action
		...
		sigma = 1.0/self.Step_Size[obs_key][act_key]*(reward + Q_next_max - Q_s_a)
		...
		for states in self.Q_Table.keys():
			for action_ in self.Q_Table[states].keys():
				self.Q_Table[states][action_] += self.Step_Size[states][action_]*sigma*self.E_Trace[states][action_]	
		 		self.E_Trace[states][action_] *= self.lambda_
 			if(self.Q_Table[states]=={}):
				self.Q_Table[states][act_key] = self.Step_Size[states][act_key]*sigma*self.E_Trace[states][act_key]
				self.E_Trace[states][act_key] *= self.lambda_
