...
class helicopter_agent(Agent):
	...
	def getNextAction(self,state):
		...
		if(self.Total_Value.has_key(state)):
			...
			if(self.randGenerator.random() > self.Epsilon):
				...
				action_random = convertListToString(self.discretize_action(self.generateRandomAction()))
				self.Total_Value[state][action_random] = 0 
				self.Q_Table[state][action_random] = 0 
				return action_random
		else:
			return convertListToString(self.discretize_action(self.generateRandomAction()))
	...
	def agent_action_step(self,reward,observation):
		...
		#get Next action according to epsilon-policy
		next_action = self.getNextAction(next_key)		
